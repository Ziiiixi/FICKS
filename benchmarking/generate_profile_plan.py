...
# MODIFIED SEMANTICS:
#   If True, we will consider the plan column "(-1,-1)" as a real profile target and ONLY
#   generate (-1,-1) rows when that cell is NaN in the plan.
ADD_NO_MASK_BASELINE = True
NO_MASK_TPC_VALUE    = -1
NO_MASK_COL_NAME     = "(-1,-1)"
...
def parse_tpc_col(col: str):
    """
    Parse a TPC column name like "(1,23)" or "(-1,-1)" -> (tpc1, tpc2).
    Supports negatives.
    """
...
def canon_tpc_col_name(col: str) -> str:
    """
    Canonicalize "( 1, 23 )" -> "(1,23)" and "(-1, -1)" -> "(-1,-1)".
    Non TPC-like columns are returned unchanged.
    """
...
# Canonicalize any "(a,b)"-style column names so spacing differences do not break detection
df.columns = [canon_tpc_col_name(c) for c in df.columns]

# If we want no-mask baseline, ensure the column exists (missing -> treated as NaN)
if ADD_NO_MASK_BASELINE and NO_MASK_COL_NAME not in df.columns:
    print(f"[INFO] Plan has no '{NO_MASK_COL_NAME}' column; creating it as NaN (will be considered missing).")
    df[NO_MASK_COL_NAME] = pd.NA
...
# ---- Strip _bzN, then normalize names ----
...
# We keep only:
#   (a) the no-mask column (-1,-1) if enabled
#   (b) columns whose parsed (tpc1,tpc2) are both >=0 and sum to TOTAL_TPCS
tpc_cols = []
tpc_map = {}
...
for c in df.columns:
    if not isinstance(c, str):
        continue
    if not re.match(r"^\(\s*-?\d+\s*,\s*-?\d+\s*\)$", c):
        continue

    t1, t2 = parse_tpc_col(c)

    if (t1, t2) == (NO_MASK_TPC_VALUE, NO_MASK_TPC_VALUE):
    ...
    if t1 >= 0 and t2 >= 0:
        if t1 + t2 == TOTAL_TPCS:
            ...
            # This might be some other "(a,b)" column; warn but do not include.
            print(f"[WARN] Ignoring column {c}: tpc1 + tpc2 = {t1 + t2} != {TOTAL_TPCS}")
        continue

# Sort tpc_cols for nicer printing and stable iteration
def _tpc_sort_key(col: str):
    a, b = tpc_map[col]
    # put (-1,-1) first
    if (a, b) == (NO_MASK_TPC_VALUE, NO_MASK_TPC_VALUE):
        return (-10**9, 0)
    return (a, -b)

tpc_cols = sorted(tpc_cols, key=_tpc_sort_key)

if not tpc_cols:
    raise RuntimeError("No valid TPC columns found in plan CSV (e.g., '(1,23)' or '(-1,-1)').")

print(f"[INFO] Found {len(tpc_cols)} TPC columns: {tpc_cols[:8]}{' ...' if len(tpc_cols) > 8 else ''}")
if ADD_NO_MASK_BASELINE:
    print(f"[INFO] No-mask baseline enabled: column '{NO_MASK_COL_NAME}' "
          f"{'FOUND' if NO_MASK_COL_NAME in tpc_cols else 'NOT FOUND in tpc_cols'}")
...
print(f"[INFO] Unique missing configs (before filtering): {len(df_missing)}")

# ---------- filter out too-small TPC configs ----------
# IMPORTANT MODIFICATION:
#   Do NOT drop (-1,-1) rows when enforcing MIN_TPC.
if ENFORCE_MIN_TPC:
    before = len(df_missing)

    is_nomask = (
        (df_missing["tpc1"] == NO_MASK_TPC_VALUE) &
        (df_missing["tpc2"] == NO_MASK_TPC_VALUE)
    )
    is_valid_min = (
    ...
    df_missing = df_missing[is_nomask | is_valid_min].copy()
...
for (m_ut, m_co), grp in df_missing.groupby(["Model_UT", "Model_CO"]):
    ...
    grp_out = grp_out.sort_values(
        by=["id1", "id2", "tpc1", "tpc2"],
    ...
    nomask_cnt = int(((grp_out["tpc1"] == NO_MASK_TPC_VALUE) & (grp_out["tpc2"] == NO_MASK_TPC_VALUE)).sum())
    print(f"[WRITE] {out_path}  ({len(grp_out)} rows, nomask_rows={nomask_cnt}) for model pair ({m_ut}, {m_co})")
...
print(f"[SUMMARY] ADD_NO_MASK_BASELINE={ADD_NO_MASK_BASELINE}, NO_MASK_TPC_VALUE={NO_MASK_TPC_VALUE}, NO_MASK_COL_NAME={NO_MASK_COL_NAME}")
