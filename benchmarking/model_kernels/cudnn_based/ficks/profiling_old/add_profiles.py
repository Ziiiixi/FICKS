...
IMPORTANT CHANGES (THIS VERSION):
  (1) We DO NOT compute knees from PLAN / EXCL curves anymore.
      We ONLY use manual knees from knee_coloc and knee_excl override files.

  (2) We ONLY use knee_tpc_new (if empty -> fill with sat_tpc_auto and write back).

  (3) "Short kernel" classification improved:
      Instead of using only excl@24 (or mean dominated by max TPC),
      we use a robust duration proxy:
        duration_proxy = mean of worst-K excl latencies over tpc >= MIN_TPC_FOR_SHORT
      (default worst-K=3)

Dirs (your current tree):
  ficks/profiling/estimated_coloc_curves/*.csv
  ficks/profiling/knee_coloc/*_est_coloc_knee_overrides.csv
  ficks/profiling/knee_excl/*_knee_overrides.csv
"""
...
# These are kept for compatibility, but NOT USED for knee selection now.
PLAN_CSV    = PROFILING_DIR / "A4000_profile_plan_all_cluster_pairs_up_to3.csv"
GEOMEAN_MULT_CSV = PROFILING_DIR / "A4000_ratios_ut.csv"
KNEE_PARAMS_JSON = PROFILING_DIR / "knee_params.json"

# Manual knees (YOU WANT THESE)
...
# SETTINGS
# ============================================================
MIN_TPC_FOR_SHORT = 1     # consider tpc>=3 for short duration proxy
WORST_K_FOR_SHORT = 20     # duration_proxy = mean of worst-K points over tpc>=MIN_TPC_FOR_SHORT

SHORT_COL_NAME = "is_short_kernel"

# debug prints
DEBUG_RANDOM_SEED = 123
...
# ============================================================
# MANUAL KNEE OVERRIDE LOADING (knee_coloc + knee_excl)
# ============================================================
def fill_knee_tpc_new_with_sat_tpc_auto_inplace(override_csv: Path) -> int:
    ...
    if "sat_tpc_auto" not in df.columns:
        # if sat_tpc_auto not present, nothing we can do
        return 0
...
def duration_proxy_worstk_from_excl(
    ...
    """
    duration proxy per kernel = mean of top-K largest excl_<tpc> values over tpc>=min_tpc

    Fixes your issue:
      - kernels slow at low TPC but fast at 24 are NOT wrongly "short"
      - robust to noise vs using max()
    """
    if not excl_cols:
    ...
    for i in range(mat.shape[0]):
        ...
        worst_vals = row_sorted[-k:]  # largest K
        out[i] = float(np.mean(worst_vals))
...
# MERGE HELPERS
# ============================================================
def attach_cluster_and_manual_knees_and_exclusive(
    csv_path: Path,
    ...
    """
    Merge cluster, add knee_coloc_n and exclusive_knee_n (MANUAL, per-kernel),
    and join exclusive per-TPC columns.
    ...
    if id_col is not None:
        ...
        # Ensure Name exists with a stable name
...
# ============================================================
# OPTIONAL: KNEE PARAM LOADING (NOT USED NOW)
# ============================================================
...
def main():
    ...
    for f in files:
        ...
        # ============================================================
        # OLD AUTO KNEE COMPUTATION (COMMENTED OUT)
        # ============================================================
        # Previously:
        #   - exclusive_knee_n computed from exclusive curves using derivative threshold
        #   - knee_coloc_n computed from PLAN curves per cluster_UT
        #
        # NOW:
        #   - exclusive_knee_n comes from knee_excl override (per-kernel)
        #   - knee_coloc_n comes from knee_coloc override (per-kernel)
        #
        # So we DO NOT compute knees here anymore.
        #
        # ------------------------------------------------------------
        # (Example of old code that is no longer used)
        #
        # coloc_p = params_for_model(sheet, model_key, defaults, models, "coloc")
        # excl_p  = params_for_model(sheet, model_key, defaults, models, "exclusive")
        #
        # id_to_excl_knee = build_exclusive_knee_map_for_df(...)
        # knee_map_coloc  = build_cluster_to_coloc_knee_map(...)
        #
        # ============================================================

        # merge + output
        try:
            updated_df, unmatched, _ = attach_cluster_and_manual_knees_and_exclusive(
                csv_path=f,
            ...
            # improved duration proxy for short classification
