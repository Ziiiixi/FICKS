...
EXCL_DIRS = [Path("/home/zixi/orion_bu/artifact_evaluation/FICKS/new_A4000_profiles")]
...
def exclusive_lookup(excl_df):
    """
    Build a LUT:
        lut[kernel_id][TPC_n] = latency   (same unit as in excl_df)
    ...
    # Pick the ID column (Kernel Index / id / kernel_id / etc.)
...
def choose_exclusive_lut_for_model(model_name: str, luts_by_file: dict):
    ...
    # prefix: filename stem starts with model_name (common: 'resnet152' → 'resnet152_bz8')
    for stem, lut in luts_by_file.items():
        if stem.lower().startswith(model_name.lower()):
            return lut

    # prefix: model_name starts with stem
    for stem, lut in luts_by_file.items():
...
def compute_ratios_for_all_rows(plan_df, model_to_lut):
    ...
    For the whole plan_df (no reordering):
      For each row:
        ...
        - kid   = row[ut_col]
        - for each '(n,m)' column:
              ratio = (L_coloc(µs?) - L_excl(µs?)) / L_excl(µs?)
      If LUT or entry is missing, ratio = NaN.
    ...
    # Plan values: assume already in same unit as exclusive (ns or µs)
    ...
    # For each config column '(n,m)' use n to pick exclusive TPC
    for col, n_ut in zip(cfg_cols, n_ut_list):
        ...
        for i in range(len(out)):
            ...
            excl_val = lut.get(kid, {}).get(n_ut, np.nan)  # same unit as plan
            coloc_val = out.iat[i, out.columns.get_loc(col)]  # same unit as plan

            if pd.isna(coloc_val) or pd.isna(excl_val) or excl_val <= 0:
                vals.append(np.nan)
            else:
                vals.append((coloc_val - excl_val) / excl_val)
        out[col] = vals
...
def step1_build_ratio_csv():
    ...
    # Build per-model LUT mapping based on Model_UT in plan
...
def step2_build_geomean_by_cluster_pair():
    ...
    # You can sort if you want deterministic ordering:
    # agg = agg.sort_values(["Model_UT", "cluster_UT", "cluster_CO"]).reset_index(drop=True)
