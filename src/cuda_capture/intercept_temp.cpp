...
extern "C" {
    void *cudnn_handle_dummy = (void*)cudnnCreate;
}
...
cudaError_t cudaMalloc(void** devPtr, size_t size) {
	...
	int idx = get_idx();
	assert (idx >= 0);
	DEBUG_PRINT("[IDX %d] Caught cudaMalloc! allocate region of %zu bytes\n", idx, size);
...
cudaError_t cudaMemcpyAsync(void* dst, const void* src, size_t count, enum cudaMemcpyKind kind, cudaStream_t stream) {
	...
	DEBUG_PRINT("[IDX: %d] Caught cudaMemcpyAsync! src=%p, dst=%p, size=%zu, stream=%p\n",
            idx, src, dst, count, (void*)stream);
