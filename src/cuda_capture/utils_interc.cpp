...
#error "SYS_gettid unavailable on this system"
    ...
    // Only accept threads that were pre-registered in thread_ids[]
    for (int i = 0; i < num_tids; i++) {
        if (tid == thread_ids[i]) {
            int idx = i % (clients + 1);

            // Only pin real client threads 0..clients-1
            if (idx >= 0 && idx < clients && !affinity_set[idx]) {
                int offset = (clients == 2) ? 4 : 1;
                cpu_set_t mask;
                CPU_ZERO(&mask);
                CPU_SET(idx + offset, &mask);
                int result = sched_setaffinity(0, sizeof(mask), &mask);
    ...
    // Unknown thread: do NOT treat as a client.
    // Return -1 so intercept wrappers will bypass queueing.
    return -1;
}
